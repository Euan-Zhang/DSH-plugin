...
2. **快速接入**（支持「手动配置」与「粘贴 cURL」两种方式）：
   - **粘贴 cURL**：把命令行里的 `curl ...` 命令粘进输入框，点「解析并填充」，插件会
     ...
     （类型自动推断、来源为「Agent 输入」、说明为「请补充中文说明」），然后切回手动
     表单供你补全名称、调用时机与参数中文说明。
   - **手动配置**：直接填写下面的字段：
   ...
   - **凭据引用**：只填一个**引用名**（环境变量名样式，例如 `WEATHER_API_KEY`），
     **不填密钥本身**。
...
API 工具只保存**凭据引用名**，实际密钥由 DSH 的凭据存储提供。常用做法是把密钥放进
目标电脑的环境变量（例如 `WEATHER_API_KEY`），或写入 DSH 的凭据存储。

- 若密钥放在**环境变量**中：DSH 凭据服务会直接解析同名环境变量，无需额外配置。
- 页面上的凭据状态提示会显示「已配置 / 未配置」，未配置时调用会明确报错。

> 安全约束：密钥不会进入普通配置、Agent 上下文或调用轨迹；接口地址仅允许 http/https；
> 响应体超过 256 KB 或超过 60 秒会中止，避免失控。
...
- **Q：测试提示「凭据 XXX 未配置」？**
  A：表示 DSH 凭据服务解析不到该引用名。请确认对应名称的环境变量已设置，或已写入 DSH
  凭据存储。
